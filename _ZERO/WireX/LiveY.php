...
trait LiveX
{
	...
	// • === paramsPropertyX → ... »
	protected function paramsPropertyX(&$input, array $keys)
	{
		foreach ($keys as $key) {
			$this->paramPropertyX($input, $key);
		}
	}
	...
	// • === onSave → ... »
	protected function oSave($id, $label, &$action, &$response = null)
	{
	...
	// • === oClone → ... »
	protected function oClone($id, $view = 'clone')
	{
		$filter = $this->filterAsBindID($id);
		$record = $this->modelX::oFindX($filter);
		if (!$record) {
			...
			$this->setPropertyX(ArrayX::stripKey($record->getAttributes(), ['guid']));
		}
		$this->cloneX($view, $record);
	}
	...
	// • === filterAsBindID → ... »
	protected function filterAsBindID($bind)
	{
		$filter = [];
		if (!empty($bind)) {
			if (is_array($bind)) {
