...
trait ModelX
{
	...
	public static function generateSN()
	{
		$lastSN = self::lastSN();
		if ($lastSN) {
			$newSN = DataX::generateSN($lastSN);
			if (!self::hasGuid($newSN)) {
				return $newSN;
			}
		}
		...
		$maxSN = self::maxSN();
		if ($maxSN) {
			$newSN = DataX::generateSN($maxSN);
			if (!self::hasGuid($maxSN)) {
				return $maxSN;
			}
		}
	...
	// • === oUpdateParentChildX → ... »
	public static function oUpdateParentChildX($input, $childModel, $parentChildMethod)
	{
		$parentRow = $input['parent'];
		$parentRow = DataX::setAmountToNullKey($parentRow, ['discount', 'holding', 'subtotal', 'vat']);
		...
		try {
			...
			$parent->{$parentChildMethod}()->delete();
	...
	public static function oFindByFilterX($filter)
	{
		if (is_array($filter)) {
